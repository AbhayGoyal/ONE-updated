 ...
 * roads of a SimMap. 
 */
public class MapBasedMovement_2_3 extends MovementModel implements SwitchableMovement {
...
//	protected int ma
	 ...
	 */
	public MapBasedMovement_2_3(Settings settings) {
		super(settings);
		map = readMap();
...
//		this.pathFinder = new DijkstraPathFinder(getOkMapNodeTypes());
	 ...
	 */	
	public MapBasedMovement_2_3(Settings settings, SimMap newMap, int nrofMaps) {
		super(settings);
		map = newMap;
	 ...
	 */
	protected MapBasedMovement_2_3(MapBasedMovement_2_3 mbm) {
		super(mbm);
	...
	public Coord getInitialLocation() {
		...
		getAllNodes(map.getNodes(), host);
//		List<MapNode> hostNodes = host.possibleNodes;
		
		////////////////////////////////////////
		
		// choose a random node (from OK types if such are defined)
		Random randNom = new Random();
		int showMe = randNom.nextInt(500);
//		do
//		{
//			if(host.name.contains("v1"))
//			{
//				System.out.println("came for" + host.name);
//				host.actualRegion = 1;
//				
//				
//				for(MapNode m:nodes)
//				{
//					if((m.getLocation().getX() >=4000 && m.getLocation().getX() < 5000) && (m.getLocation().getY()>= 3800 && m.getLocation().getY() < 4300))
//						n = m;
//				}
//			}
//		}
...
//		while();
		do 
		{
			...
			///assign pillar to the regions
			
			if(host.name.contains("v2"))
			{
				System.out.println("came for" + host.name);
				host.actualRegion = 1;
				for(MapNode m:nodes)
				{
					if((m.getLocation().getX() >=200+showMe && m.getLocation().getX() < 6000+showMe) && (m.getLocation().getY()>= 200+showMe && m.getLocation().getY() < 6000+showMe))
					{
						n = m;
					}
			...
			if(!host.name.contains("v2"))
			{
				...
				for(MapNode m:nodes)
				{
					if((m.getLocation().getX() >=200+showMe && m.getLocation().getX() < 1000+showMe) && (m.getLocation().getY()>= 3800+showMe && m.getLocation().getY() < 5400+showMe))
					{
...
//			
//			if(host.name.contains("v3"))
//			{
//				host.actualRegion = 3;
//				for(MapNode m:nodes)
//				{
//					if((m.getLocation().getX() >=-100+showMe && m.getLocation().getX() < 2000+showMe) && (m.getLocation().getY()>= -300+showMe && m.getLocation().getY() < 2000+showMe))
//					{
//						n = m;
//					}
//						
...
//			}
//			
//			if(host.name.contains("v4"))
//			{
//				host.actualRegion = 4;
//				for(MapNode m:nodes)
//				{
//					if((m.getLocation().getX() >=4000+showMe && m.getLocation().getX() < 6000+showMe) && (m.getLocation().getY()>= 100+showMe && m.getLocation().getY() < 600+showMe))
//					{
//						n = m;
//					}
//						
...
//			}
//			
//			if(host.name.contains("v5"))
//			{
//				host.actualRegion = 3;
//				for(MapNode m:nodes)
//				{
//					if((m.getLocation().getX() >=200 && m.getLocation().getX() < 5200) && (m.getLocation().getY()>= 300 && m.getLocation().getY() < 6200))
//					{
//						n = m;
//					}
////						n = m;
...
//			int[] regs = {1, 2, 3, 4};
//			int regionValue = rng.nextInt(regs.length);
...
//		n2 = host.possibleNodes.get(rng.nextInt(host.possibleNodes.size()));
	...
	public List<MapNode> getAllNodes(List<MapNode> allNodes, DTNHost host)
	{
		List<MapNode> lst = new ArrayList<>();
		int r = host.actualRegion;
//		if(s.contains("v1"))
		
		System.out.println("hosts run successfully");
		...
		for(MapNode n:allNodes)
		{
			...
			if ((c.getX() > 0 && c.getX() <= 754) && (c.getY() > 0 && c.getY() <= 700))
			{
				...
				{
					System.out.println("came here");
					host.possibleNodes.add(n);
				}
			}
			else if ((c.getX() > 4524 && c.getX() <= 5078) && (c.getY() > 4200 && c.getY() <= 4800))
			{
				if(r == 2)
					host.possibleNodes.add(n);
			}
			else if ((c.getX() > 1508 && c.getX() <= 2262)&& (c.getY() > 1400 && c.getY() <= 2100))
			{
				if(r == 3)
					host.possibleNodes.add(n);
			}
			else if ((c.getX() > 2262 && c.getX() <= 3016) && (c.getY() > 2100 && c.getY() <= 2800))
			{
				if(r == 4)
					host.possibleNodes.add(n);
			}
			
			else if ((c.getX() > 3016 && c.getX() <= 3720) && (c.getY() > 2800 && c.getY() <= 3500)) 
			{
				if(r == 5)
					host.possibleNodes.add(n);
			}
			else if ((c.getX() > 3720 && c.getX() <= 4524) && (c.getY() > 3500 && c.getY() <= 4200))
			{
				if(r == 6)
					host.possibleNodes.add(n);
			}
	...
	public Path getPath() {
		...
		System.out.println("we came in method");
...
//			neighbors = host.possibleNodes;
			...
			if (okMapNodeTypes != null) 
			{ //remove neighbor nodes that aren't ok
...
//				while(true)
//				{
//					MapNode m = n2.get(rng.nextInt(n2.size()));
//					
//					if(host.name.contains("v1"))
//					{
//						if((m.getLocation().getX() >=1000 && m.getLocation().getX() < 6000) && (m.getLocation().getY()>= 1000 && m.getLocation().getY() < 5500))
//						{
//							nextNode = m;
////							System.out.println("stuck in infinte");
//							break;
//						}
//					}
//				}
...
//				nextNode = n2.get(rng.nextInt(n2.size()));
				...
				System.out.println(lastMapNode + " -> " + nextNode);
				System.out.println(lastMapNode.getLocation() + " " + nextNode.getLocation());
//				host.possibleNodes.add(nextNode);
	...
	@Override
	public MapBasedMovement_2_3 replicate() {
		return new MapBasedMovement_2_3(this);
	}
