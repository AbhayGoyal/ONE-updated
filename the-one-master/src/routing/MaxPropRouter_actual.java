 ...
 */
public class MaxPropRouter_actual extends ActiveRouter {
    /** Router's setting namespace ({@value})*/
	public static final String MAXPROP_NS = "MaxPropRouter_actual";
	/**
	...
	public HashMap<Message, Double> allMessagesTimestamps;
	 ...
	 */
	public MaxPropRouter_actual(Settings settings) {
		super(settings);
	 ...
	 */
	protected MaxPropRouter_actual(MaxPropRouter_actual r) {
		super(r);
		this.alpha = r.alpha;
		...
		this.allMessagesTimestamps = new HashMap<>();
	}
	...
	public void changedConnection(Connection con) {
		...
		if (con.isUp()) { // new connection
			...
			if (con.isInitiator(getHost())) {
				...
				assert mRouter instanceof MaxPropRouter_actual : "MaxProp only works "+
				" with other routers of same type";
				MaxPropRouter_actual otherRouter = (MaxPropRouter_actual)mRouter;
	...
	protected void transferDone(Connection con) {
		...
		// System.out.println("came to transfer done");
...
// 
	...
	public void update() {
		...
		// if(SimClock.getTime() > 8000 && SimClock.getTime() %1000 == 0)
		// {
		// 	System.gc();
		...
		// Get the snapshot of the system at every second
		System.out.println("For host " + getHost().name + " at time"+ SimClock.getTime() +"connected hosts are" + this.getConnections().size()+"\n");
	...
	private Tuple<Message, Connection> tryOtherMessages() 
	{
		...
		for (Connection con : getConnections()) 
		{
			...
			MaxPropRouter_actual othRouter = (MaxPropRouter_actual)other.getRouter();
			...
			for (Message m : msgCollection) 
			{
				...
				////////////////////////////////////////////////
				// if(m.to.name.equals(other.name))
				// {
				// 	System.out.println("Message is" + m.id + "hops" + m.path.size() + "latency" + (SimClock.getTime() - m.timeCreated));
	...
	private class MaxPropComparator implements Comparator<Message> {
		 ...
		 * {@link MaxPropRouter_actual#getCost(DTNHost, DTNHost)}) is first.
		 */
	...
	public MessageRouter replicate() {
		MaxPropRouter_actual r = new MaxPropRouter_actual(this);
		return r;
	}
}
