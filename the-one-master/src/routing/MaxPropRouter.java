...
//import routing..combine;
//import routing.CustomRouter.locs;
import routing.CustomRouter.timeVals;
...
//					return;
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		if (isDeliveredMessage(m)) {
			...
			String filePath = "outputInterStats_MaxProp.txt";
...
//	            dataBuild.append("Delivered till now is" + msr.nrofDelivered +"created till now is " + msr.nrofCreated +" \n");
	            dataBuild.append("all probs are "+this.allProbs);
	...
	protected Message getNextMessageToRemove(boolean excludeMsgBeingSent) {
		...
		// List<Message> first30Messages = new ArrayList<>();
//		messages.subList(0, Math.min(30, messages.size()));
		...
		// for (Message m : messages) 
		// {
		// 	first30Messages.add(m);
		// }
		// }

//		if(messages.size() > 30)
//			messages = messages.subList(0, 30);
		// ArrayList messages_list = m 
...
//		System.out.println(Double.toString(currSpeed) + " " + currTime + " " + currlocation + " " + toLocation);
		...
		// adding.delete(0, adding.length());

//		System.out.println("profile is"+dtnHost.allMessages);

//		System.out.println("name is" + dtnHost.name);

//		Process p = Runtime.getRuntime().exec("python script.py");

//		System.out.println("message is" + dtnHost.msgListeners);

//		findMin n = new findMin();
//
...
//			host.region4 += 0.25;
	...
	public void update() 
	{
		// String filePath = "outputInterConnectStats_MaxProp.txt";
...
//            String data = "Hello, world!\nThis is some data written to a file.";
            ...
            List<Connection> all = getConnections();
            
            dataBuild.append("All connections present at "+ SimClock.getTime() +" are : " + all.size() + "\n");
            
            for(Connection each:all)
            {
                dataBuild.append("The connections are now from "+ each.fromNode.name + "to " + each.toNode.name+"\n");
            }
...
//		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// gc.collect();
		// if(SimClock.getTime() >= 8000 && SimClock.getTime() % 500 == 0)
		// {
		// 	System.gc();
		...
		if (SimClock.getTime() % 900 == 0 && !this.getHost().name.contains("p")) 
		{
			System.out.println("Came for" + getHost().name);
...
//				"{"time":{"0":"9-10"},"probs":{"0":[0.5,["p29","p10"]]}}"
	...
	private Tuple<Message, Connection> tryOtherMessages() 
	{
		...
		for (Connection con : getConnections()) 
		{
			...
			getHost().interactionsMade++;
			other.interactionsMade++;
			...
			for (Message m : msgCollection) 
			{
				...
				//////////////////////////////////////////////////////
				if(SimClock.getTime() - m.timeReceived > 10)
...
//							System.out.println("region is"+ host.pillar_region_connection.keySet());
					...
					if(st.contains(other.actualRegion))
						messages.add(new Tuple<Message, Connection>(m,con));
...
//		System.exit(0);
	...
	public Set<Integer> getKeysByValue(Map<Integer, List<DTNHost>> pillar_region_connection, String loc2_s) 
	{
	    ...
	    for (Map.Entry<Integer, List<DTNHost>> entry : pillar_region_connection.entrySet()) 
	    {
...
//			System.out.println("mesg1 hops" + msg1+"is" +hopc1 + "msg2 hops" +msg2+"is"+ hopc2);
			 ...
			 * be sent first */
			else if (hopc1 < threshold && hopc2 < threshold) {
				return hopc1 - hopc2;
			}
			else if (hopc1 > threshold && hopc2 > threshold) {
				return hopc2 - hopc1;
...
//			}
			...
			p2 = getCost(from2, msg2.getTo());
			 ...
			 int costComparison = Double.compare(p1, p2);
		        if (costComparison != 0) {
		            return costComparison; // compare by cost
		        }

		        // If costs are equal, compare by hop count
		      return Integer.compare(hopc1, hopc2);
