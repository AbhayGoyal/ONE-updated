...
public abstract class ActiveRouter extends MessageRouter {
	...
	public class MessageStored
	{
		Message msg;
		Double time;
		public MessageStored(Message m, Double time)
		{
			this.msg = m;
			this.time = time;
		}
	...
	protected Tuple<Message, Connection> tryMessagesForConnected(List<Tuple<Message, Connection>> tuples) 
	{
		...
		for (Tuple<Message, Connection> t : tuples) 
		{
			...
			if(m.to.name.equals(otherHost.name))
			{
				// System.out.println("Message is" + m.id + "hops" + m.path.size() + "latency" + (SimClock.getTime() - m.timeCreated));
			...
			if (startTransfer(m1, con) == RCV_OK) 
			{
				// System.out.println("putting now" + " " + m.id +" " + SimClock.getTime());
				otherHost.allMessagesTimestamps.add(new MessageStored(m, SimClock.getTime()));
////////////////////////////////////////////////////////////////////////////////////////
				// String filePath = "/home/aghnw/data/the-one-master/messagesBuffer_RF+GAE_70_14400.txt";
			
				// String filePath_creation = "/home/aghnw/data/the-one-master/messagesCreationTimes.txt";
		// 		try 
		// 		{
		...
		// 			FileWriter fileWriter_creation = new FileWriter(filePath_creation, true);

		// 			// Create a BufferedWriter for efficient writing
		// 			BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);

		// 			BufferedWriter bufferedWriter_creation = new BufferedWriter(fileWriter_creation);

		// 			// Data to write to the file
		// //            String data = "Hello, world!\nThis is some data written to a file.";
		// 			StringBuilder dataBuild = new StringBuilder();

		// 			StringBuilder dataBuild_creation = new StringBuilder();
					
		// 			ArrayList<MessageStored> bef= getHost().allMessagesTimestamps;
					

		// 			for(MessageStored msg : bef)
		// 			{
		// 				Message msgGot= msg.msg;
		// 				Double pastTime = msg.time;
		// 				if(pastTime == null)
		// 					pastTime = 0.0;
		// 				dataBuild.append(msgGot.id+" "+ getHost().name +" "+ (SimClock.getTime() -  pastTime)+ "\n");
		// 			}

		// 			for(MessageStored msg : bef)
		// 			{
		// 				Message msgGot= msg.msg;
		// 				// Double creationTime = msg.time;
		// 				// if(pastTime == null)
		// 				// 	pastTime = 0.0;
		// 				dataBuild_creation.append(msgGot.id+" "+ msgGot.timeCreated +" \n");
		// 			}
					

		// 			String data = dataBuild.toString();

		// 			String data_creation = dataBuild_creation.toString();

		// 			// Write the data to the file
		// 			bufferedWriter.write(data);

		// 			bufferedWriter_creation.write(data_creation);

		// 			dataBuild.delete(0, dataBuild.length());

		// 			// Close the BufferedWriter and FileWriter to release resources
		// 			bufferedWriter.close();
		// 			fileWriter.close();

		// 			bufferedWriter_creation.close();
		// 			fileWriter_creation.close();
